...
var opponents = huskyGames2016.map(extractOpponent);
...
function formatGame(game) {
  ...
  if (game.home === "UW") {
    return `${opponent} at UW, ${game.opponent_score} to ${game.home_score}`;
  }
  else {
    return `UW at ${opponent}, ${game.home_score} to ${game.opponent_score}`;
  }
}
...
function hadFumble(game) {
  return game.fumbles > 0;
}
var fumbleGames = huskyGames2016.filter(hadFumble);
...
var fumbledAndLostFilter = makeCombinedFilter(huskiesLost, hadFumble);
...
var lostWithFumbles = huskyGames2016.filter(fumbledAndLostFilter);
...
var totalScoreDifference = huskyGames2016.reduce(function(accumulator, game) {
  let scoreDifference;
  if (game.home === "UW") {
    scoreDifference = game.home_score - game.opponent_score;
  } else {
    scoreDifference = game.opponent_score - game.home_score;
  }
  return accumulator + scoreDifference;
}, 0);

var avgScoreDifference = totalScoreDifference / huskyGames2016.length;
